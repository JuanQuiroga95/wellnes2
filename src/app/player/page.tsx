...
export default async function PlayerPage() {
  ...
  const [jRows, logs, wRows, todayRows] = await Promise.all([
    sql`SELECT u.nombre, j.posicion, j.edad, j.peso_kg::text AS peso_kg, j.estatura_cm, j.pie_habil FROM usuarios u JOIN jugadores j ON j.usuario_id=u.id WHERE u.id=${session.userId}`,
  ...
  const j = jRows[0] ? { posicion:String(jRows[0].posicion||''), edad:Number(jRows[0].edad)||null, peso_kg:String(jRows[0].peso_kg||''), estatura_cm:Number(jRows[0].estatura_cm)||null, pie_habil:String(jRows[0].pie_habil||'') } : null
