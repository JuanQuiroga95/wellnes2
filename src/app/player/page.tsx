...
import { calcACWR, buildACWRHistory } from '@/lib/acwr'
...
export default async function PlayerPage() {
  ...
  const [jRows, logs, wRows, todayRows] = await Promise.all([
    sql`SELECT u.nombre, j.posicion, j.edad, j.peso_kg::text AS peso_kg, j.estatura_cm, j.pie_habil, j.foto_url FROM usuarios u JOIN jugadores j ON j.usuario_id=u.id WHERE u.id=${session.userId}`,
  ...
  const j = jRows[0] ? { posicion:String(jRows[0].posicion||''), edad:Number(jRows[0].edad)||null, peso_kg:String(jRows[0].peso_kg||''), estatura_cm:Number(jRows[0].estatura_cm)||null, pie_habil:String(jRows[0].pie_habil||''), foto_url:jRows[0].foto_url?String(jRows[0].foto_url):null } : null
  ...
  return <PlayerClient session={session} jugador={j} jugadorId={jugadorId} acwr={calcACWR(sl)} acwrHistory={buildACWRHistory(sl)} recentLogs={rl} recentWellness={wRows.map(pw)} todayWellness={todayRows[0]?pw(todayRows[0]):null} today={today} />
}
